        ...
        for action in [Directions.NORTH, Directions.SOUTH, Directions.EAST, Directions.WEST]:
            ...
            if not hitsWall:
                ...
                if next_node in self.corners and next_node not in visited_corners:
                    n_visited_corners = visited_corners + (next_node,)
                    successor = ((next_node, n_visited_corners),action,1)
...
def cornersHeuristic(state: Any, problem: CornersProblem):
    ...
    short_length = min(walls.width-3,walls.height-3)
    long_length = max(walls.width-3,walls.height-3)
    h_sum = 0
    if len(state[1]) == 4:
        return 0
    ...
    for corner in corners:
        if corner not in state[1]:
            ...
            if len(state[1]) == 0:
                tmp+=2*short_length+long_length
            elif len(state[1]) == 1:
                left_corners = [c for c in corners if c not in state[1] and c != corner]
                if util.manhattanDistance(left_corners[0],left_corners[1])>2*short_length:
                    tmp +=2*short_length+long_length
                else:
                    tmp +=short_length+long_length
            elif len(state[1]) == 2:
                left_corners = [c for c in corners if c not in state[1]]
                tmp+=util.manhattanDistance(left_corners[0],left_corners[1])
